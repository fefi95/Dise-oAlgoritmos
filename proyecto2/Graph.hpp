...
std::pair < std::vector<Edge>,std::vector<int> > findCycleFrom(int v1, std::vector< vector<int> > path, std::vector<Edge> graph) {
    std::vector<int> eulerian;
    // Find adjacent to source vertex
    for(std::vector<int>::iterator adj0 = path[v1].begin(); adj0 != path[v1].end(); ++adj0) {
        int u = v1;  //source
        int vi = *adj0;
        bool visited = false;
    ...
    std::pair < std::vector<Edge>,std::vector<int> > result(graph, eulerian);
    return result;
}
...
std::pair < int,std::vector<int> > getPath(std::vector< vector<int> > path, std::vector<Edge> graph){
    ...
    std::pair < std::vector<Edge>,std::vector<int> > cycle;
    cycle = findCycleFrom(0, path, graph);
    std::vector<int> eulerian = cycle.second;
    graph = cycle.first;
    ...
    // std::cout << "not visited edges" << std::endl;
    // for(std::vector<Edge>::iterator edge = graph.begin(); edge != graph.end(); ++edge) {
    //     t = edge -> get_times();
    //     v1 = edge -> get_v1() - 1;
    //     if (t == 0){
    ...
    //         std::pair < std::vector<Edge>,std::vector<int> > cycle;
    //         cycle = findCycleFrom(v1, path, graph);
    //         std::vector<int> new_eulerian;
    //         std::cout << "new" << std::endl;
    //         for (int vi = 0; vi < eulerian.size(); vi++) {
    //             new_eulerian.push_back(eulerian[vi]);
    //             std::cout << eulerian[vi];
    //             if (eulerian[vi + 1] == cycle.second[0]){
    //                 for (int vj = 0; vj < cycle.second.size(); ++vj) {
    //                     new_eulerian.push_back(cycle.second[vj]);
    //                     std::cout << cycle.second[vj];
    //                 }
    //             }
    //         }
